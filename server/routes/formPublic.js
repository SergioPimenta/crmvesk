...
router.get('/:code.js', async (req, res) => {
  ...
  const widget = await getFormWidgetByMonitorCode(code);
  if (!widget) {
    return res.status(404).type('text/plain').send('// rastreador não encontrado ou inativo');
  }
...
router.post('/:code/submit', async (req, res) => {
  ...
  try {
    const result = await submitFormLead(code, req.body ?? {});
    res.json(result);
  } catch (err) {
    res.status(400).json({ message: err.message || 'Erro ao registrar envio' });
  }
});
