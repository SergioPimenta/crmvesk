...
export async function createTemplate(userId, { nome, descricao, buffer, filename, mimeType }) {
  ...
  const [result] = await pool.query(
    `INSERT INTO proposal_templates (user_id, nome, descricao, file_url, file_name, mime_type, file_size)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [userId, cleanNome, String(descricao || '').trim(), fileUrl, filename || '', mimeType || '', buffer.length]
  );

  return { id: String(result.insertId) };
}

export async function renameTemplate(userId, id, { nome, descricao }) {
  const cleanNome = String(nome || '').trim();
  if (!cleanNome) throw new Error('Informe o nome do modelo');

  const [result] = await pool.query(
    'UPDATE proposal_templates SET nome = ?, descricao = ? WHERE id = ? AND user_id = ?',
    [cleanNome, String(descricao || '').trim(), id, userId]
  );
